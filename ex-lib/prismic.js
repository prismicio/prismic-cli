...
import initSlices from './nuxt-slices'
    ...
    getUsage([
      ...
      },
      {
        ...
        content: [
          {
            name: '$ prismic init',
            summary: 'Create a project from an existing prismic repository.'
          },
          {
            name: '$ prismic new',
            summary: 'Create a project with a new prismic repository.'
          },
          {
            name: '$ prismic init foobar',
            summary: 'Create a project for the foobar repository'
          },
          {
            name:
              '$ prismic init foobar --folder ~/Desktop/myProject --template NodeJS --noconfirm',
            summary: 'Create a NodeJS project, non-interactive'
          },
          {
            name: '$ prismic theme https://github.com/prismicio/nodejs-sdk',
            summary:
        ...
        content: [
          {
            name: 'quickstart',
            summary:
              'Create a project: initialize a node.js quickstart project with a new prismic repository.'
          },
          {
            name: 'init',
            summary:
              'Initialize a project: initialize the code from a template for an existing prismic repository.'
          },
          {
            name: 'new',
            summary:
              'Create a project: initialize the code for a new prismic repository.'
          },
          {
            name: 'theme',
            summary:
              'Create a project: initialize project from a theme with a new prismic repository.'
          },
          {
            name: 'login',
            summary: 'Login to an existing prismic.io account.'
          },
          {
            name: 'logout',
            summary: 'Logout from an existing prismic.io account.'
          },
      ...
      },
      {
        ...
        optionList: [
          ...
          {
            name: 'password',
            description: 'The password of the account to use.'
          },
          {
            name: 'folder',
            description: 'The folder to create the new project.'
          },
          {
            name: 'template',
            description:
              'Project template to use (see the list command for available templates).'
          },
          {
            name: 'noconfirm',
            description:
              'Prevent the interactive mode. Fails if informations are missing.'
          },
          {
            name: 'conf',
            description:
              'Specify path to prismic configuration file. Used with `theme` command.'
          },
          {
            name: 'ignore-conf',
            description:
              'Ignores prismic configuration file. Used with `theme` command.'
          }
        ]
      }
    ])
...
async function signup(config, args) {
  ...
  Helpers.UI.display(
    'Successfully created your account! You can now create repositories.'
  )
}
...
async function list() {
  ...
  const templates = await Helpers.Prismic.templates()
  Helpers.UI.display(
    Template.getDisplayed(templates).map(template => `* ${template.name}`)
  )
}
...
function main() {
  ...
  if (args['--debug']) configuration.set({ debug: true })
  configuration.getAll().then(config => {
    switch (command) {
      case 'slice-machine':
        initSlices(config, args)
        break
      case 'slicemachine':
        initSlices(config, args)
        break
        ...
        Helpers.Prismic.templates().then(templates => {
          ...
          if (t) {
            init(
              config,
              firstArg,
              Object.assign(args, { '--new': true, '--template': t.name })
            )
          } else {
