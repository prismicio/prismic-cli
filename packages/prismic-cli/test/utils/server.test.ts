...
describe('server', () => {
  ...
  beforeEach(async () => {
    server = Server.build('https://prismic.io', 5555, 'localhost')
  ...
  it('returns a 404 for all other methods', async () => {
  ...
  it('handles valid POST request', async () => {
    ...
    // expect(fakeOnSuccess.args[0], 'callback should be called with correct args').to.deep.equal([{cookies: fakeCookie, email: fakeEmail}, `Logged n as ${fakeEmail}`])
  });
