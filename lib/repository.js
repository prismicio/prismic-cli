...
async function create(templates, base, domain, args, theme) {
  ...
  const template = theme ? Promise.resolve(theme.template) : await chooseTemplate(d, templates, args, noconfirm);
...
async function chooseTemplate(domain, templates, args, noconfirm) {
  function prompt(template) {
    ...
    .then((answers) => Template.get(templates, answers.template));
  }
...
async function readZipAndCreateRepoWithCustomTypes(newRepository, base, domain, args, template, folder, theme, noconfirm) {
  const tmpfolder = theme ? Promise.resolve(theme.tmpFolder) : await Template.unzip(template.url);
