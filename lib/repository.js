...
async function validateTheme(themeURL) {
  ...
  if(themeData) {
    ...
    if(isValidConfig) {
      ...
      retry(themeURL, 'Invalid theme. You should have a \'prismic-configuration.js\' with the prismic apiEndpoint');
    }
  } else {
    retry(themeURL, 'Invalid theme URL provided.');
  }
}
...
function isGithubRepository(url) {
  ...
  if (matches) {
    const zipUrl = `${url}/archive/master.zip`;
    const innerFolder = `${matches[1]}-master`;
    return {url: zipUrl, name: matches[1], innerFolder};
  } else null;
}
...
async function readZipAndCreateRepoWithCustomTypes(newRepository, base, domain, args, template, folder, noconfirm) {
  const tmpfolder = await Template.unzip(template.url);
...
function templateQuestion(templates, templateName) {
  const displayTemplates = templates.filter(t => !t.isQuickstart);
  return {
    type: 'list',
    name: 'template',
...
function promptThemeURL(themeURL) {
  return inquirer.prompt([themeURLQuestion(themeURL)]);
}
