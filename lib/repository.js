...
async function create(base, domain, args) {
  ...
  if (!finalDomain) {
    finalDomain = await promptForNewDomain(base, domain);
  }
  ...
  if (newRepository) {
    const customTypes = readCustomTypes(path.join(tmpfolder, template.innerFolder));
  ...
  shell.mv(template.innerFolder, folder);
...
function readCustomTypes(folder) {
  if (folder) {
    let customTypesPath = path.join(folder, 'customtypes.json');
    if (shell.test('-e', customTypesPath)) {
      return JSON.stringify(JSON.parse(shell.cat(customTypesPath)));
    }
  }
  return {};
}

async function promptForNewDomain(base, domain) {
  ...
  if (doExists) {
    ...
    return promptForNewDomain(base, domain);
  } else {
