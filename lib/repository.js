...
async function validateTheme(themeURL) {
  ...
  try {
    ...
    if(themeData) {
      ...
      if(isValidConfig) {
        return Helpers.Theme.make(themeData.name, themeData.url, tmpFolder);
      } else {
...
function isZipURL(url) {
  ...
  const matches = url.match(regexp);
  if (matches) return {url, name: matches[1]};
  else null;
