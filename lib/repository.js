...
function createWithDomain(base, domain, args, noconfirm) {
  return new Promise((resolve, reject) => {
    ...
    .then(function (cookies) {
      ...
      .then(() => {
        Helpers.UI.display(`You can now create content from your the Writing Room here: ${Domain.withDomain(base, domain)}`);
        resolve(domain);
      })
      .catch((err) => {
        reject();
...
function create(base, domain, args) {
  ...
  return new Promise((resolve, reject) => {
    if (domain) {
      exists(base, domain, args, noconfirm)
      .then((isExist) => {
      ...
      promptName().then((answers) => {
        exists(base, answers.domain, args, noconfirm)
        .then((isExist) => {
          if(isExist) resolve(answers.domain);
          else reject(`We didn't create the new repository ${answers.domain}`);
        });
      });
    }
  })
...
function exists(base, domain, args, noconfirm) {
  return new Promise((resolve, reject) => {
    ...
    .then(function (exists) {
      ...
      if (!isExists) {
        if(noconfirm) {
          ...
          .then((answers) => {
            if(answers.createIfNotExist) createWithDomain(base, domain, args, noconfirm).then((d) => resolve(true));
...
function promptCreateIfNotExists() {
  return inquirer.prompt([{
    ...
    message: 'This repository doesn\'t exist. Do you want to create it?'
  }]);
}
