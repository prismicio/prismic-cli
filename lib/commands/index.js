  ...
  Helpers.Cmd.apply({
    ...
    context: {
      ...
      template: (async () => {
        ...
        const quickstartTmpl = templates.find(
          (tmpl) => tmpl.isQuickstart === true,
        );
...
async function sync() {
  await Sync();
  Helpers.UI.display('Successfully synced!');
}
...
export async function run(cmdName, args = []) {
  ...
  switch (cmd.alias) {
    case Commands.init.alias:
      return init();
    case Commands.sliceMachine.alias:
      return slicemachine();
    case Commands.login.alias:
      return login();
    case Commands.logout.alias:
      return logout();
    case Commands.signup.alias:
      return signup();
    case Commands.newProject.alias:
      return init();
    case Commands.quickstart.alias:
      return init();
    case Commands.theme.alias:
      return initWithTheme();
    case Commands.heroku.alias:
      return heroku();
    case Commands.list.alias:
      return list();
    case Commands.base.alias:
      return updateBase();
    case Commands.sync.alias:
      return sync();
    default: {
