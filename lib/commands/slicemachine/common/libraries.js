...
function infos(libPath) {
  ...
  const pathToConfig = path.posix.join(
    pathToLib,
    globals.DEFAULT_SM_CONFIG_PATH,
  );
...
function slicesModels(lib) {
  ...
  return paths.reduce(
    (acc, p) => ({
      ...acc,
      ...buildSliceModel(lib, /* libPath */ pathToSlices, isLocal, p),
    }),
    {},
  );
}
...
function createSliceLibMap(prev, curr) {
  const updated = { ...prev };
  Object.keys(curr).forEach((key) => {
    const item = curr[key];
    updated[item.id] = item.lib.replace(/^(@|~)/, '');
  });
  return updated;
}

export async function getProjectSlices(
  smFile,
  includeLibMap = false,
) /* { [sliceId]: sliceModel } */ {
  const modelsByLib = await Promise.all(
    smFile.libraries.map((lib) => slicesModels(lib)),
  );
  const libMap = includeLibMap && modelsByLib.reduce(createSliceLibMap, {});
  ...
  const slices = modelsByLib.reverse().reduce((acc, currentSlices) => {
    const updatedSlicesModels = Object.entries(currentSlices)
      .map(([, { id, model }]) => ({
        [id]: model,
      }))
      .reduce((objAcc, model) => ({ ...objAcc, ...model }), {});

    return { ...acc, ...updatedSlicesModels };
  }, {});
  return includeLibMap ? { libMap, slices } : slices;
}
