...
function _sliceModel(libPath, isLocal, slicePath) {
  ...
  return {
    [sliceName]: {
      id, lib: libPath, path: path.normalize(slicePath), model, isLocal,
    },
  };
}
...
async function slicesModels(libPath) {
  try {
    ...
    const models = await (async () => {
      ...
      return paths.reduce((acc, p) => ({ ...acc, ..._sliceModel(libPath, isLocal, p) }), {});
    })();
