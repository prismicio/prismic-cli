...
import { pascalize, snakecalize } from 'sm-commons/utils/str';
...
async function createLocal(framework, sliceName, modelWithPath, pathToSlices, pathToTemplates) {
  const field = findRichTextField(modelWithPath.model);
  await copy(
...
async function createRemote(framework, prismicEndpoint, sliceName) {
  try {
