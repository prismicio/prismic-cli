...
function findRichTextField(modelWithPath) {
  const DEFAULT = 'false';
  if (!modelWithPath || !modelWithPath.model) return DEFAULT;

  const { 'non-repeat': fields = {} } = modelWithPath.model;
  const maybeEntry = Object.entries(fields).find(([, e]) => e.type === 'StructuredText');
  return maybeEntry ? `slice.primary.${maybeEntry[0]}` : DEFAULT;
}

async function createLocal(framework, sliceName, modelWithPath, pathToSlices, pathToTemplates) {
  const field = findRichTextField(modelWithPath);
  await copy(
    path.join(pathToTemplates, framework),
...
async function createRemote(framework, prismicEndpoint, sliceName, templatesPath) {
  try {
    ...
    const sliceModel = (() => {
      const defaultModelFile = fs.readFileSync(path.join(templatesPath, framework, 'model.json'), 'utf8');
