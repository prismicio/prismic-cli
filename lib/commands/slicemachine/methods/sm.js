...
function validate(smConfig) {
  ...
  if (!smConfig.libraries) {
    throw new Error(
      'sm file should have a "libraries" field, filled with paths to SM libs',
    );
  }
...
function write(data) {
  const apiEndpoint = typeof data.apiEndpoint === 'string'
    ? data.apiEndpoint.toLowerCase()
    : data.apiEndpoint;
  const str = JSON.stringify({ ...data, apiEndpoint });
  try {
    const { pathToSmFile } = getFilePath();
    return fs.writeFileSync(pathToSmFile, str, 'utf8');
  } catch (e) {
    return consola.error(
      `Could not write "${globals.DEFAULT_SM_CONFIG_PATH}" file`,
    );
  }
}
...
function patch(data, invert = false) {
  try {
    ...
    return consola.error(
      `Could not patch "${globals.DEFAULT_SM_CONFIG_PATH}" file`,
    );
  }
}

function buildCustomTypes(templates, slices) {
  return (templates || []).map(({ mergeSlices = true, ...template }) => {
    const computedCustomType = CustomTypes.mergeSlices(
      template.value,
      mergeSlices ? slices : {},
    );
...
export {
  buildCustomTypes, get, getOrFail, validate, write, patch,
};
