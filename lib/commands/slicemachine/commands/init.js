...
async function init() {
  ...
  const configFile = Mustache.render(readFileSync(path.join(__dirname, '../misc', 'templates', 'prismic.config.mustache'), 'utf8'), {
    apiEndpoint: info.apiEndpoint,
  });
  writeFileSync('prismic-configuration.json', configFile, 'utf8');
  ...
  (function handleSmFile() {
    ...
    console.log({
      ...
      local: ctx.SliceMachine.localSlicesPath,
