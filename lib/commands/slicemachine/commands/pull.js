...
const Displays = {
  ...
  async createMissingSlices(framework, missing) {
    if (!missing) return;
    ...
    for (let i = 0; i < missingEntries.length; i += 1) {
      ...
      await Slices.createLocal(framework, sliceName, { path: missingSlice.path, model: missingSlice.model }, libPath, path.join(__dirname, '../templates'));
    }
  },
...
export default async function () {
  try {
    ...
    CustomTypes.write(customTypes, customType => ({
      id: ct.mask_id, name: ct.mask_name, repeatable: ct.mask_repeatable, value: ct.mask_value,
    }));
    ...
    Object.entries(slicezoneByCustomTypes).forEach(([customTypeId, slicezones]) => {
      Object.entries(slicezones).forEach(async ([slicezoneId, slices]) => {
        ...
        // NEW SLICES
        await Displays.createMissingSlices(framework, miss);
