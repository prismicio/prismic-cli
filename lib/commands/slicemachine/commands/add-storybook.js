...
import { transform } from 'babel-transform-config';
...
export default async function () {
  ...
  if (manifest.useConfig) {
    ...
    console.log(manifest.useConfig(storiesMatch));
    writeFileSync(path.join(process.cwd(), `bck.${manifest.configFile}`), transformedConfig.code);
    process.exit(0);
