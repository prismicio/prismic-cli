...
const Framework = {
  next: {
    ...
    useConfig: false,
    useTemplate: true,
    devDependencies: ['@storybook/react@5.3.19', 'babel-loader', 'babel-plugin-react-require'],
    storyExtension: '[tj]s',
    scripts: {
    ...
    extendProject() {
      ...
      if (existsSync(babelrcPath)) {
        ...
        if (!babelrc.contains('react-require')) {
        ...
        writeFileSync(babelrcPath, JSON.stringify({ plugins: ['react-require'] }, null, 2));
      }
    },
    command: 'storybook',
  },
...
export default async function () {
  ...
  shell.exec(`${packageManager.cmd} run ${manifest.command} -- --smoke-test`);
}
