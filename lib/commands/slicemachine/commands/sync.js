...
const Displays = {
  ...
  async createMissingSlices(missing) {
    ...
    if (selectedSlices.length === 0) returnl;

    const libPath = await Libraries.selectLocal();
    const missingWithmeta = Objects.map(missing, ([id, model]) => {
      const name = Slice.nameFromId(id);
      return Slice.apply(id, name, model, /* isLocal */ true, libPath);
    });

    console.log(missingWithmeta);
  },
