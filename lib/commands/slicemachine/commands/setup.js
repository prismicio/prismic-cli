...
import {
  get as getSmFile,
  write as writeSmFile,
  buildCustomTypes,
} from '../methods/sm';
import { getProjectSlices } from '../common/libraries';
...
function generateProjectConfig(manifest, info) {
  try {
    ...
    const configFile = Files.read(
      path.join(process.cwd(), manifest.configPath),
    );
    const { code: updatedConfigFile } = transformConfig(
      configFile,
      manifest.framework,
      manifest,
    );
    ...
    consola.success(
      `I rewrote your ${manifest.frameworkName} configuration file!`,
    );
    consola.info('After trying SliceMachine, make sure to check it out!');
    return;
  } catch (e) {
    consola.error(
      'An unexpected error occured while rewriting configuration file',
    );
  }
}
...
function handleRecap(manifest, info) {
  ...
  const sliceZoneReadmeUrl = () => {
    switch (manifest.framework) {
      case 'nuxt':
        return 'https://prismic.io/docs/technologies/vue-slicezone-technical-reference';
      case 'next':
        return 'https://github.com/prismicio/slice-machine/tree/master/packages/next-slicezone';
      default:
        return 'https://github.com/prismicio/slice-machine/';
    }
  };
  ...
  const quickStartDocsUrl = () => {
    switch (manifest.framework) {
      case 'nuxt':
        return 'https://prismic.io/docs/technologies/nuxtjs';
      case 'next':
        return 'https://prismic.io/docs/technologies/quick-start-nextjs';
      default:
        return 'https://slicemachine.dev/documentation/getting-started';
    }
  };
  ...
  const addSlicesDocsUrl = () => {
    switch (manifest.framework) {
      case 'nuxt':
        return 'https://prismic.io/docs/technologies/generate-model-component-nuxtjs';
      case 'next':
        return 'https://prismic.io/docs/technologies/create-your-own-slices-components-nextjs';
      default:
        return 'https://www.slicemachine.dev/';
    }
  };
  ...
  const vars = {
    ...
    writingRoomUrl: info.apiEndpoint
      ? `${info.apiEndpoint.split('/api')[0].replace('cdn.', '')}/documents`
      : 'https://prismic.io/dahboard',
...
async function setup() {
  try {
    ...
    if (yes) {
      consola.info(
        'Running in no-confirm mode: some files might be overridden.',
      );
    }
    ...
    if (!smFile) {
      ...
      const {
        manifest,
        customTypes: customTypesTemplates,
      } = await loadBootstrapData(framework);
      const newSmFile = {
