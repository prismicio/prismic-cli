...
function _readPrismicEndpoint() {
  if (!(ctx.endpoint || ctx.domain)) {
    try {
      const file = fs.readFileSync(path.join(process.cwd(), './prismic-configuration.js'), 'utf8');
...
async function createRemoteSlice(framework, sliceName) {
  ...
  try {
    ...
    return Communication.postJson(`${Helpers.Endpoints.slicesApi(prismicEndpoint)}/create`, opts);
  } catch (e) {
    Sentry.report(e);
    console.log(new Error(`Cannot create the slice ${sliceName} in Prismic.`))
    throw new Error(`[full error] ${e}`)
  }
