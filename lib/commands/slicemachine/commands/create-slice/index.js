...
function findRichTextField(model) {
  ...
  const [key] = Object.entries(fields).find(e => e.type === 'StructuredText')
  return maybeEntry ? `slice.primary.${key}`: 'false'
}
...
export default async function () {
  ...
  try {
    const { model } = await createRemoteSlice(framework, sliceName);
