...
async function createPrismicRepo(config, args, customTypes) {
  ...
  await Authentication.connect(ctx);
  const createdDomain = await Repository.createWithDomain({
    base: config.base,
    domain,
    args,
    customTypes,
    context: 'slice-machine',
  });
  Helpers.UI.display('--');
...
async function init(config, args) {
  ...
  try {
    ...
    const maybeZipPath = await SliceMachine.downloadSlices(endpoint, { framework, ...(args['--demo'] ? { demo: true } : {}) });
    if (maybeZipPath && maybeZipPath instanceof Error === false) {
      ...
      if (!args['--skip-prismic'] && (info.isBootstrapped || shell.test('-e', protocol.prismicConfig) === false || await promptForPrismicApp())) {
        ...
        const urls = writePrismicConfig(zip, protocol.prismicConfig, { domain, base: config.base });
