...
export function isSameVersionOrHigher(local, remote) {
  const distTags = remote['dist-tags'];
  const { latest, ...unstable } = distTags;
  const prerelease = unstable ? Object.values(unstable) : [];
  const highestStableRemoteVersion = Object.keys(remote.versions)
    .filter((version) => (semver.valid(semver.coerce(version)) === version) && prerelease.includes(version) === false)
