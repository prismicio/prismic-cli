...
async function createWithDomain({ domain, customTypes, signedDocuments }) {
  try {
    ...
    Helpers.UI.display(`You can access your backend here: ${magicUrl}`);
    return domain;
  } catch (error) {
    Sentry.report(error, 'create-repository-with-domain');
...
async function exists(base, domain) {
  try {
    ...
    Sentry.report(err, 'check-repository-name');
...
async function chooseTemplate(templates) {
  ...
  if (ctx.Themes.template) {
    try {
      ...
      Sentry.report(error, 'get-template-from-existing');
...
async function heroku(templates, templateName) {
  const answers = (async () => {
    try {
      ...
      Sentry.report(err, 'init-repository-with-theme');
