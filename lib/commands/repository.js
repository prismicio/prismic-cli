...
async function chooseTemplate(domain, templates) {
  async function prompt(template) {
    const answers = await promptTemplate(templates, template);
    return answers.template;
  }

  if (ctx.Theme.template) {
    const temp = Template.get(templates, ctx.Theme.template);
    if (!temp) {
      Helpers.UI.display(`Error: invalid template ${ctx.Theme.template}`);
...
async function create(templates, theme) {
  ...
  const template = theme ? theme.template : await chooseTemplate(selectedDomain, templates);
