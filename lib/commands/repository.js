...
function queryCreateRepositoryWithCookie({
  domain, customTypes, signedDocuments, ctx,
}) {
  ...
  return Communication.post(url, data, ctx.cookies);
}
...
async function createWithDomain({
  ...
  try {
    await (async () => {
      ...
      const cookies = await Authentication.connect(ctx);
      return queryCreateRepositoryWithCookie({
        domain, customTypes, signedDocuments, ctx,
      });
    })();

    Helpers.UI.display(`You can access your backend here: ${await Helpers.MagicLink.buildRedirectUrl(ctx.base, domain)}`);
    const magicUrl = await Helpers.MagicLink.buildRedirectUrl(ctx.base, domain);
    Helpers.UI.display(`Redirecting you to: ${await Helpers.MagicLink.buildRedirectUrl(ctx.base, domain)} ...`);
    open(magicUrl, { wait: true });
    return domain;
  } catch (error) {
    if (error.statusCode === 401) {
      // remove cookie
      return LocalDB.set({ cookies: '' }).then(() => (
        createWithDomain({
          domain, customTypes, signedDocuments, ctx,
        })
      ));
...
async function isDomainAvailableOrRetry(ctx, maybeDomain) {
...
async function chooseDomain(ctx) {
  if (domain) {
    return isDomainAvailableOrRetry(ctx);
  } else if (noconfirm) {
    throw new Error('The noconfirm options requires the domain option to be set.');
  } else {
    const answers = await promptName();
    return isDomainAvailableOrRetry(ctx, answers.domain);
  }
}
...
async function chooseFolder(domain, ctx) {
  ...
  if (folder) {
    if (shell.test('-e', folder)) {
      ...
      if (!ctx.noConfirm) prompt(folder);
    } else {
      return folder;
    }
  } else {
...
function promptTemplate(templates, templateName) {
  ...
  return inquirer.prompt([{
    ...
    default: findIndex(propEq('name', templateName))(templateName),
    choices: map(displayTemplates, template => ({ name: template.name, value: template })),
  }]);
}

async function chooseTemplate(domain, templates, ctx) {
  async function prompt(template) {
    const answers = await promptTemplate(templates, template);
    return answers.template;
  }

  const template = ctx.Theme.template;
  if (template) {
...
async function readZipAndCreateRepoWithCustomTypes(domain, template, folder, theme) {
  selectedDomain, template, folder, theme;
...
async function create(templates, ctx, theme) {
  const selectedDomain = await chooseDomain(ctx);
  const folder = await chooseFolder(selectedDomain, ctx);
  const template = theme ? theme.template : await chooseTemplate(selectedDomain, templates, ctx);
