...
async function createWithDomain({ domain, customTypes, signedDocuments }) {
  try {
    await (async () => {
      ...
      const result = await queryCreateRepositoryWithCookie({ domain, customTypes, signedDocuments });
      if (typeof result !== 'string' && result.statusCode && result.statusCode !== 200) {
        const e = new Error(`Repository creation failed. Status: ${result.statusCode}`);
    ...
    if (error.statusCode === 401 || error.statusCode === 303) {
      // remove cookie
      setCookies('');
      return createWithDomain({ domain, customTypes, signedDocuments });
    }
...
function promptName(domain) {
  return inquirer.prompt([{
    ...
    transformer(domainInput) {
      const msg = [
        chalk.dim('https://'),
        domainInput ? chalk.cyan(domainInput) : chalk.dim.cyan('repo-name'),
        chalk.dim('.prismic.io'),
      ];
      return msg.join('');
    },
    validate(value) {
      ...
      const lowercaseOnly = value.toLowerCase() === value;
      if (!lowercaseOnly) errors.push('No uppercase letters.');

      const startsWithLetter = /^[a-z]/.test(value);
      if (!startsWithLetter) errors.push('Sub-domain must start with a letter.');

      const acceptedChars = /^[a-z0-9-]+$/.test(value);
      if (!acceptedChars) errors.push('Most contain only letters, numbers and hyphens.');

      const moreThanFourCharacters = value.length >= 4;
      if (!moreThanFourCharacters) errors.push('Must have at least four alphanumeric characters and/or hyphens.');

      const endsWithALetterOrNumber = /[a-z0-9]$/.test(value);
      ...
      // const regexp = /^[a-z][a-z0-9-]{2,}[a-z0-9]$/;

      return `Validation errors: ${errorString}`;
    },
  }]);
}
...
async function chooseDomain() {
  if (ctx.domain) { return isDomainAvailableOrRetry(); }
  if (ctx.noConfirm) {
...
function readCustomTypes(folder) {
  if (folder) {
    const customTypesFolder = path.join(folder, ctx.Themes.customTypes || 'custom_types');
...
function readDocuments(folder) {
  ...
  if (folder) {
    const docsFolder = path.join(folder, ctx.Themes.documents || 'documents');
    ...
    if (shell.test('-e', metaPath)) {
      ...
      const docs = langIds.reduce((docByLangAcc, langId) => {
        ...
        const docsForLang = docsFilename.reduce((docAcc, docFilename) => {
          ...
          return { ...docAcc, [docName]: docValue };
        }, {});

        return { ...docByLangAcc, ...docsForLang };
      }, {});
