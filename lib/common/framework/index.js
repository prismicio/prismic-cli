...
export function detect() /* ProjectType */ {
  ...
  if (!packageJson && !bowerJson) {
    console.log('[Error] Could not find package.json nor bower.json files. Are you in the right folder?')
    return null;
  }
...
export function parse(strFramework) {
  if (Object.entries(ProjectType).some(([, value]) => value === strFramework.toLowerCase())) {
    return strFramework.toLowerCase()
  }
  return null
}
