...
function makeTemplate(name, url, configuration, instructions, isQuickstart) {
  const config = configuration || DEFAULT_CONFIG_PATH;
  return {name, url, configuration: config, instructions, isQuickstart: isQuickstart || false};
}
...
export default {
  ...
  Theme: {
    ...
    make(name, url, tmpFolder) {
      return {tmpFolder, template: makeTemplate(name, url)};
    }
  },
  Json: {
    merge(obj, other) {
