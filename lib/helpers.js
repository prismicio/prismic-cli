...
const Helpers = {
  ...
  Domain: {
    default: globals.DEFAULT_DOMAIN,
    repository(base, domain) {
      const matches = (base || globals.DEFAULT_DOMAIN).match(new RegExp('((https?://)([^/]*))'));
  ...
  MagicLink: {
    ...
    async buildRedirectUrl(base, domain) {
      const token = await ctx.Auth.magicLink;
      const redirectUri = Helpers.Domain.repository(base, domain);
      if (token) {
        return `${(base || globals.DEFAULT_DOMAIN)}/magic?token=${token}&redirectUri=${redirectUri}`;
      }
      return redirectUri;
    },
  },
  ...
  Cmd: {
    ...
    fromName(cmdName, commands) {
      return toPairs(commands).reduce((acc, [cmdAlias, cmd]) => {
        if (cmd.names.includes(cmdName)) return cmd;
        else return acc;
      }, null);
    },
  }
