...
import { signinWithCredentials, signinWithOAuth } from './login';
...
async function signupOrLogin(base) {
  ...
  if (answers.login === 'login') {
    return signinWithCredentials(base);
  }
  return Signup(base);
}
...
async function connect(base, args, noconfirm) {
  ...
  const oauthService = args['--oauthservice'];
  ...
  } else if (oauthService && oauthAccessToken) {
    await signinWithOAuth(base, oauthService, oauthAccessToken);
    cookiesPromise = config.get('cookies');
  } else if (email && password) {
    // The user included login/password, we need to log him with those
    await signinWithCredentials(base, email, password);
    cookiesPromise = config.get('cookies');
  } else {
    if (noconfirm) {
