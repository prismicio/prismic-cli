...
import Endpoints from '../misc/createEndpoints';
...
export default async (/* config */c, args) => {
  try {
    ...
    const baseParams = { framework: 'nuxt', ...(args['--demo'] ? { demo: true } : {}) }
    ...
    const [missingSlices, toFetchFromSlicemachine] = await (async () => {
      ...
      const slicemachineSlices = (await SliceMachine.fetchAllSlicesModels(endpoint, baseParams, cookies))
