...
function handleActions(zip, actions, isBootstrapped) {
  actions.forEach((action) => {
    const { type: actionType, zipPath, writePath } = action;
    ...
    if (shouldWrite) {
      zip.extractEntryTo(
        ...
        './',
        true,
        shouldOverWrite(action, isBootstrapped),
      );
...
function recap(protocol, info) {
  ...
  const { newPrismicRepo, prismicDomain } = info;
  shell.exec('clear');

  console.log('available data', { ...protocol, ...info });
  const additionalDisplay = Mustache.render(protocol.additionalDisplay, {
    ...protocol,
    ...info,
    documentsUrl: newPrismicRepo.documentsUrl,
  }).replace('<REPOSITORY>', prismicDomain);
