...
async function init(config, domain, args, theme) {
  const base = config.base !== undefined ? config.base : Helpers.Domain.default;
...
function main() {
  const validCommands = [null, 'init', 'new', 'theme', 'quickstart', 'heroku', 'slice-machine', 'slicemachine', 'sm', 'login', 'logout', 'signup', 'base', 'version', 'list'];
  ...
  return configuration.getAll().then((config) => {
    switch (command) {
      case 'slice-machine':
